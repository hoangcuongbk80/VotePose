        ...
        {          
          ...
          if(overlap > overlap_min) 
          {
            ...
            *model_cloud += *model_transf;
            ...
            save_file << object_name << " " << row(0) << " " << row(1) << " " << row(2) << " ";
...
int main(int argc, char **argv)
{
  ...
  nh_.getParam("object_name", object_name); 
  ...
  while (ros::ok())
  {
      ...
      if(method==2) // assign semantic class id, instacnce id, and part id and save poses to gt-txt
      {
        loadClould_black();
        assign_ids();
